...
Future<void> loadDynamicProductData() async {
  ...
  try {
    ...
    print('ðŸ” Loading dynamic data with admin ID: ${adminId}');
    ...
    if (response.statusCode == 200) {
      ...
      if (data['success'] == true && data['pages'] != null) {
        ...
        print('âœ… Loaded ${productCards.length} dynamic products');
    ...
    print('âŒ Error loading dynamic data: $e');
...
void startRealTimeUpdates() async {
  ...
  if (adminId != null) {
    ...
    _updateSubscription = _appSync.updates.listen((update) {
      ...
      print('ðŸ“± Received real-time update: $type');
...
class SessionManager {
  ...
  static Future<void> initFromAdminConfig({
    ...
    print('ðŸ” Admin config loaded: $loadedAppName');

    print('ðŸŽ¨ App name set globally: ${SessionManager.appName}');
  ...
  static Future<void> bindAuth({
    ...
    print('ðŸ” Session bound to userId: $userId');
...
class AdminManager {
  ...
  static Future<String> getCurrentAdminId() async {
    ...
    assert(
      ...
      'âŒ CRITICAL: Admin ID override detected',

    );

    _currentAdminId = adminId;

    print('âœ… Admin ID locked: $adminId');
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _fetchAppNameAndNavigate() async {
    ...
    try {
      ...
      print('ðŸ” Splash screen using admin ID: ${adminId}');
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ...
          print('âœ… Splash screen loaded app name: ${_appName}');

        }

      } else {

        print('âš ï¸ Splash screen API error: ${response.statusCode}');
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadDynamicAppConfig() async {
    ...
    try {
      ...
      print('ðŸ” Home page using admin ID: ${adminId}');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          print('âœ… Loaded ${_dynamicProductCards.length} products from backend');
                ...
                GestureDetector(
                  ...
                  onTap: () {
                    ...
                    setState(() {
                      ...
                      _currentPageIndex = 3; // Navigate to profile page
...
appBar: AppBar(
  ...
  Future<Map<String, dynamic>> _fetchUserProfile() async {
    try {
      ...
      // Return fallback data instead of empty map
      return {
        'firstName': 'User',
        'lastName': '',
